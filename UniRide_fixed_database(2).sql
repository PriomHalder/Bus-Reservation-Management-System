...
--
-- Host: 127.0.0.1
-- Generation Time: Aug 19, 2026 at 09:52 PM
...
DROP DATABASE IF EXISTS `bus_reservation_system`;
...
-- --------------------------------------------------------
-- Missing support tables restored from the application logic
-- --------------------------------------------------------

CREATE TABLE `booking_status_history` (
  `history_id` int(11) NOT NULL AUTO_INCREMENT,
  `booking_id` int(11) NOT NULL,
  `old_status` enum('BOOKED','CONFIRMED','CANCELLED','COMPLETED','TRANSFER_PENDING') DEFAULT NULL,
  `new_status` enum('BOOKED','CONFIRMED','CANCELLED','COMPLETED','TRANSFER_PENDING') NOT NULL,
  `changed_by` enum('SYSTEM','PASSENGER','UNIVERSITY_ADMIN','ADMIN') NOT NULL DEFAULT 'SYSTEM',
...
CREATE TABLE `semester_bills` (
  ...
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  PRIMARY KEY (`bill_id`),
  UNIQUE KEY `uq_semester_bill_passenger_semester` (`passenger_id`,`semester_id`),
...
CREATE TABLE `ticket_transfers` (
  ...
  KEY `idx_ticket_transfers_sender` (`from_passenger_id`),
  KEY `idx_ticket_transfers_recipient_status` (`to_passenger_id`,`status`),
  CONSTRAINT `chk_ticket_transfer_different_passengers`
    CHECK (`from_passenger_id` <> `to_passenger_id`),
  CONSTRAINT `chk_ticket_transfer_sale_amount`
    CHECK ((`transfer_type` = 'SELL' AND `sale_amount` >= 0) OR (`transfer_type` = 'GIFT' AND `sale_amount` = 0))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
--
DROP VIEW IF EXISTS `v_passenger_booking_history`;

CREATE ALGORITHM=UNDEFINED VIEW `v_passenger_booking_history`  AS SELECT `bk`.`booking_id` AS `booking_id`, `bk`.`booking_reference` AS `booking_reference`, `bk`.`passenger_id` AS `passenger_id`, `bk`.`schedule_id` AS `schedule_id`, `bk`.`slot_type` AS `slot_type`, `bk`.`seat_number` AS `seat_number`, `bk`.`standing_slot` AS `standing_slot`, `fn_seat_label`(`bk`.`seat_number`) AS `seat_label`, `bk`.`fare_charged` AS `fare_charged`, `bk`.`qr_token` AS `qr_token`, `bk`.`status` AS `status`, `bk`.`hidden_from_passenger` AS `hidden_from_passenger`, `bk`.`booking_date` AS `booking_date`, `s`.`schedule_date` AS `schedule_date`, `s`.`departure_time` AS `departure_time`, `s`.`arrival_time` AS `arrival_time`, `r`.`route_name` AS `route_name`, `r`.`start_location` AS `start_location`, `r`.`end_location` AS `end_location`, `b`.`registration_number` AS `registration_number`, `b`.`bus_type` AS `bus_type`, `u`.`name` AS `university_name`, `u`.`university_id` AS `university_id` FROM ((((`bookings` `bk` join `schedules` `s` on(`bk`.`schedule_id` = `s`.`schedule_id`)) join `routes` `r` on(`s`.`route_id` = `r`.`route_id`)) join `buses` `b` on(`s`.`bus_id` = `b`.`bus_id`)) join `universities` `u` on(`r`.`university_id` = `u`.`university_id`)) ;
...
CREATE ALGORITHM=UNDEFINED VIEW `v_schedule_availability`  AS SELECT `s`.`schedule_id` AS `schedule_id`, `s`.`schedule_date` AS `schedule_date`, `s`.`departure_time` AS `departure_time`, `s`.`arrival_time` AS `arrival_time`, `s`.`status` AS `schedule_status`, `r`.`route_id` AS `route_id`, `r`.`route_code` AS `route_code`, `r`.`route_name` AS `route_name`, `r`.`start_location` AS `start_location`, `r`.`end_location` AS `end_location`, `r`.`fare` AS `fare`, `u`.`university_id` AS `university_id`, `u`.`name` AS `university_name`, `b`.`bus_id` AS `bus_id`, `b`.`registration_number` AS `registration_number`, `b`.`bus_type` AS `bus_type`, `b`.`seat_capacity` AS `seat_capacity`, `b`.`standing_capacity` AS `standing_capacity`, (select count(0) from `bookings` where `bookings`.`schedule_id` = `s`.`schedule_id` and `bookings`.`slot_type` = 'SEAT' and `bookings`.`status` in ('BOOKED','CONFIRMED','TRANSFER_PENDING')) AS `booked_seats`, `b`.`seat_capacity`- (select count(0) from `bookings` where `bookings`.`schedule_id` = `s`.`schedule_id` and `bookings`.`slot_type` = 'SEAT' and `bookings`.`status` in ('BOOKED','CONFIRMED','TRANSFER_PENDING')) AS `available_seats`, (select count(0) from `bookings` where `bookings`.`schedule_id` = `s`.`schedule_id` and `bookings`.`slot_type` = 'STANDING' and `bookings`.`status` in ('BOOKED','CONFIRMED','TRANSFER_PENDING')) AS `booked_standing`, `b`.`standing_capacity`- (select count(0) from `bookings` where `bookings`.`schedule_id` = `s`.`schedule_id` and `bookings`.`slot_type` = 'STANDING' and `bookings`.`status` in ('BOOKED','CONFIRMED','TRANSFER_PENDING')) AS `available_standing`, round((select count(0) from `bookings` where `bookings`.`schedule_id` = `s`.`schedule_id` and `bookings`.`slot_type` = 'SEAT' and `bookings`.`status` in ('BOOKED','CONFIRMED','TRANSFER_PENDING')) / `b`.`seat_capacity` * 100,1) AS `seated_occupancy_percent` FROM (((`schedules` `s` join `routes` `r` on(`s`.`route_id` = `r`.`route_id`)) join `buses` `b` on(`s`.`bus_id` = `b`.`bus_id`)) join `universities` `u` on(`r`.`university_id` = `u`.`university_id`)) ;
...
--
DROP VIEW IF EXISTS `v_semester_transport_charges`;

CREATE ALGORITHM=UNDEFINED VIEW `v_semester_transport_charges`  AS SELECT `sb`.`bill_id` AS `bill_id`, `sb`.`passenger_id` AS `passenger_id`, `sb`.`semester_id` AS `semester_id`, `sem`.`semester_name` AS `semester_name`, `sem`.`start_date` AS `start_date`, `sem`.`end_date` AS `end_date`, `sem`.`is_active` AS `is_active`, `sb`.`total_charges` AS `total_charges`, `sb`.`total_credits` AS `total_credits`, `sb`.`net_balance` AS `net_balance` FROM (`semester_bills` `sb` join `semesters` `sem` on(`sb`.`semester_id` = `sem`.`semester_id`)) ;
...
CREATE ALGORITHM=UNDEFINED VIEW `v_university_dashboard_stats`  AS SELECT `u`.`university_id` AS `university_id`, (select count(0) from `buses` where `buses`.`university_id` = `u`.`university_id`) AS `total_buses`, (select count(0) from `buses` where `buses`.`university_id` = `u`.`university_id` and `buses`.`status` = 'ACTIVE') AS `active_buses`, (select count(0) from `routes` where `routes`.`university_id` = `u`.`university_id`) AS `total_routes`, (select count(0) from `routes` where `routes`.`university_id` = `u`.`university_id` and `routes`.`status` = 'ACTIVE') AS `active_routes`, (select count(0) from `passengers` where `passengers`.`university_id` = `u`.`university_id`) AS `total_passengers`, (select count(0) from `passengers` where `passengers`.`university_id` = `u`.`university_id` and `passengers`.`passenger_type` = 'STUDENT') AS `total_students`, (select count(0) from `passengers` where `passengers`.`university_id` = `u`.`university_id` and `passengers`.`passenger_type` = 'FACULTY') AS `total_faculty`, (select count(0) from (`bookings` `bk` join `passengers` `p` on(`bk`.`passenger_id` = `p`.`passenger_id`)) where `p`.`university_id` = `u`.`university_id`) AS `total_bookings`, (select count(0) from (`bookings` `bk` join `passengers` `p` on(`bk`.`passenger_id` = `p`.`passenger_id`)) where `p`.`university_id` = `u`.`university_id` and `bk`.`status` in ('BOOKED','CONFIRMED','TRANSFER_PENDING')) AS `active_bookings`, (select count(0) from `complaints` where `complaints`.`university_id` = `u`.`university_id` and `complaints`.`status` in ('OPEN','IN_PROGRESS')) AS `pending_complaints` FROM `universities` AS `u` ;
...
ALTER TABLE `buses`
  ...
  ADD UNIQUE KEY `university_id` (`university_id`,`registration_number`);
...
ALTER TABLE `favorite_routes`
  ...
  ADD UNIQUE KEY `passenger_id` (`passenger_id`,`route_id`),
  ADD KEY `route_id` (`route_id`);
...
ALTER TABLE `notifications`
  ...
  ADD KEY `passenger_id` (`passenger_id`,`is_read`);
...
ALTER TABLE `routes`
  ...
  ADD UNIQUE KEY `university_id` (`university_id`,`route_code`);
...
ALTER TABLE `schedules`
  ...
  ADD KEY `schedule_date` (`schedule_date`,`departure_time`),
  ADD KEY `route_id` (`route_id`),
...
--
ALTER TABLE `billing_transactions`
  ADD CONSTRAINT `billing_transactions_ibfk_1` FOREIGN KEY (`passenger_id`) REFERENCES `passengers` (`passenger_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `billing_transactions_ibfk_2` FOREIGN KEY (`semester_id`) REFERENCES `semesters` (`semester_id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `billing_transactions_ibfk_3` FOREIGN KEY (`booking_id`) REFERENCES `bookings` (`booking_id`) ON DELETE SET NULL ON UPDATE CASCADE;

--
-- Constraints for table `bookings`
--
ALTER TABLE `bookings`
  ADD CONSTRAINT `bookings_ibfk_1` FOREIGN KEY (`passenger_id`) REFERENCES `passengers` (`passenger_id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `bookings_ibfk_2` FOREIGN KEY (`schedule_id`) REFERENCES `schedules` (`schedule_id`) ON UPDATE CASCADE;

--
-- Constraints for table `buses`
--
ALTER TABLE `buses`
  ADD CONSTRAINT `buses_ibfk_1` FOREIGN KEY (`university_id`) REFERENCES `universities` (`university_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `bus_route_assignments`
--
ALTER TABLE `bus_route_assignments`
  ADD CONSTRAINT `bus_route_assignments_ibfk_1` FOREIGN KEY (`bus_id`) REFERENCES `buses` (`bus_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `bus_route_assignments_ibfk_2` FOREIGN KEY (`route_id`) REFERENCES `routes` (`route_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `complaints`
--
ALTER TABLE `complaints`
  ADD CONSTRAINT `complaints_ibfk_1` FOREIGN KEY (`passenger_id`) REFERENCES `passengers` (`passenger_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `complaints_ibfk_2` FOREIGN KEY (`university_id`) REFERENCES `universities` (`university_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `faculty`
--
ALTER TABLE `faculty`
  ADD CONSTRAINT `faculty_ibfk_1` FOREIGN KEY (`passenger_id`) REFERENCES `passengers` (`passenger_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `favorite_routes`
--
ALTER TABLE `favorite_routes`
  ADD CONSTRAINT `favorite_routes_ibfk_1` FOREIGN KEY (`passenger_id`) REFERENCES `passengers` (`passenger_id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `favorite_routes_ibfk_2` FOREIGN KEY (`route_id`) REFERENCES `routes` (`route_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `notifications`
--
ALTER TABLE `notifications`
  ADD CONSTRAINT `notifications_ibfk_1` FOREIGN KEY (`passenger_id`) REFERENCES `passengers` (`passenger_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `passengers`
--
ALTER TABLE `passengers`
  ADD CONSTRAINT `passengers_ibfk_1` FOREIGN KEY (`university_id`) REFERENCES `universities` (`university_id`) ON UPDATE CASCADE;

--
-- Constraints for table `routes`
--
ALTER TABLE `routes`
  ADD CONSTRAINT `routes_ibfk_1` FOREIGN KEY (`university_id`) REFERENCES `universities` (`university_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `schedules`
--
ALTER TABLE `schedules`
  ADD CONSTRAINT `schedules_ibfk_1` FOREIGN KEY (`route_id`) REFERENCES `routes` (`route_id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `schedules_ibfk_2` FOREIGN KEY (`bus_id`) REFERENCES `buses` (`bus_id`) ON UPDATE CASCADE;

--
-- Constraints for table `students`
--
ALTER TABLE `students`
  ADD CONSTRAINT `students_ibfk_1` FOREIGN KEY (`passenger_id`) REFERENCES `passengers` (`passenger_id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `university_users`
--
ALTER TABLE `university_users`
  ADD CONSTRAINT `university_users_ibfk_1` FOREIGN KEY (`university_id`) REFERENCES `universities` (`university_id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- Constraints for restored support tables
ALTER TABLE `booking_status_history`
  ADD CONSTRAINT `booking_status_history_ibfk_1`
    FOREIGN KEY (`booking_id`) REFERENCES `bookings` (`booking_id`)
    ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `semester_bills`
  ADD CONSTRAINT `semester_bills_ibfk_1`
    FOREIGN KEY (`passenger_id`) REFERENCES `passengers` (`passenger_id`)
    ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `semester_bills_ibfk_2`
    FOREIGN KEY (`semester_id`) REFERENCES `semesters` (`semester_id`)
    ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `ticket_transfers`
  ADD CONSTRAINT `ticket_transfers_ibfk_1`
    FOREIGN KEY (`booking_id`) REFERENCES `bookings` (`booking_id`)
    ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `ticket_transfers_ibfk_2`
    FOREIGN KEY (`from_passenger_id`) REFERENCES `passengers` (`passenger_id`)
    ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `ticket_transfers_ibfk_3`
