...
INSERT INTO `bookings` (`booking_id`, `booking_reference`, `passenger_id`, `schedule_id`, `route_id`, `slot_type`, `seat_number`, `standing_slot`, `fare_charged`, `qr_token`, `status`, `booking_date`, `created_at`, `current_owner_id`) VALUES ('1', 'University ticket', '1', '1', '1', 'SEAT', '1', NULL, '150', 'Bracu Bus', 'BOOKED', current_timestamp(), current_timestamp(), '1');



INSERT INTO `billing_transactions` (`transaction_id`, `passenger_id`, `semester_id`, `booking_id`, `transaction_type`, `amount`, `description`, `transaction_date`) VALUES ('1', '1', '1', '1', 'BOOKING_CHARGE', '150', 'Bracu Bus ticket', current_timestamp());
...
-- =====================================================
-- SHAKIB FEATURE TABLES (merged into uniride2.sql)
-- =====================================================

-- =====================================================
-- SHAKIB FEATURE UPDATE
-- Complaint system, weekday favorite routes,
-- and student ticket marketplace
-- =====================================================
...
CREATE TABLE IF NOT EXISTS route_day_favorites (
    ...
    UNIQUE KEY unique_passenger_route_day
    (passenger_id, route_id, day_of_week),
    INDEX idx_route_day_passenger (passenger_id),
    INDEX idx_route_day_route (route_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS ticket_marketplace_posts (
    ...
    INDEX idx_marketplace_booking (booking_id),
    INDEX idx_marketplace_buyer (buyer_id),
...
ADD CONSTRAINT fk_bookings_route_id
...
-- =====================================================
-- UniRide route schedule favorites enhancement
-- =====================================================
CREATE TABLE IF NOT EXISTS schedule_favorites (
    favorite_id INT AUTO_INCREMENT PRIMARY KEY,
    passenger_id INT NOT NULL,
    route_id INT NOT NULL,
    schedule_id INT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_passenger_route_schedule (passenger_id, route_id, schedule_id),
    INDEX idx_schedule_favorite_passenger (passenger_id),
    INDEX idx_schedule_favorite_route (route_id),
    INDEX idx_schedule_favorite_schedule (schedule_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
