...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_archive_booking_history` (IN `p_booking_id` INT, IN `p_passenger_id` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cancel_booking` (IN `p_booking_id` INT, IN `p_passenger_id` INT)   BEGIN
    DECLARE v_status VARCHAR(20);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_create_booking` (IN `p_passenger_id` INT, IN `p_schedule_id` INT, IN `p_slot_type` VARCHAR(10), IN `p_slot_number` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_mark_all_notifications_read` (IN `p_passenger_id` INT)   BEGIN
    UPDATE notifications SET is_read = 1, read_at = NOW() WHERE passenger_id = p_passenger_id AND is_read = 0;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_request_ticket_transfer` (IN `p_booking_id` INT, IN `p_from_passenger_id` INT, IN `p_to_passenger_id` INT, IN `p_transfer_type` VARCHAR(10), IN `p_sale_amount` DECIMAL(10,2))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_respond_ticket_transfer` (IN `p_transfer_id` INT, IN `p_recipient_passenger_id` INT, IN `p_action` VARCHAR(10))   BEGIN
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_seat_label` (`p_seat_number` INT) RETURNS VARCHAR(5) CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci DETERMINISTIC BEGIN
